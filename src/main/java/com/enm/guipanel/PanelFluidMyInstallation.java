...
import com.enm.info.InfoStringGui;
...
public class PanelFluidMyInstallation implements GuiPanel
{
	...
	//public String classTag = this.getClass().getName();
	//InfoStringGui infogui;
	
	boolean fluxmAV, counterAV, switchAV, storageAV, fnameAV;
	int fluxmC, counterC, switchC, storageC, comentC, fnameC;
	
	@Override
	public void init(GuiContainer terminal)
	{
		//infogui = new InfoStringGui();
		...
		fnameAV =  Config.conf.getBoolean("Fluid Name info label is always visible", "general", false, "");
		...
		fnameC = Tools.HTMLColorStringToInt(Config.conf.getString("Fluid Name info label Color", "general", "0xFFFF00", ""));
	}

	@Override
	public void draw()
	{
		gui_terminal.mc.renderEngine.bindTexture(ResourceLocationRegister.texture_fluid_guitool2);
		for(int m_x = 0; m_x < map.map.length; ++m_x)
		{
			...
			{	
				if(map.map[m_x][m_y] > 0 || gui_terminal.tertminal_te.guiinfo.editmode)
				{
					...
					{
						gui_terminal.setColorRGBA(100, 100, 100, 255);
					}
					else
					{
		...
		for(int m_x = 0; m_x < map.map.length; ++m_x)
		{
			...
			{	
				if(map.map[m_x][m_y] > 0 && !gui_terminal.tertminal_te.guiinfo.editmode)
				{
						...
						if(!switchAV)
						{
							if( m_x == mouse_mappos.x && m_y == mouse_mappos.y)//select item
							{
								if(map.maplink[m_x][m_y] != null)
								{
									...
									if(te_r != null && te_r instanceof Machine_FluidValve)
									{
										gui_terminal.drawString(fontRendererObj, ((Machine_FluidValve)te_r).GetSyntaxe(), m_x*20 + gui_terminal.tertminal_te.guiinfo.map_pos.x+ 20, m_y*20 + gui_terminal.tertminal_te.guiinfo.map_pos.y + 20, switchC);
									}
								}
							}
						}
						...
						if(!fluxmAV)
						{
							if(m_x == mouse_mappos.x && m_y == mouse_mappos.y)//select item
							{
								if(map.maplink[m_x][m_y] != null)
								{
									...
									if(te_r != null && te_r instanceof Machine_FluidFlowMeter)
									{
										gui_terminal.drawString(fontRendererObj, ((Machine_FluidFlowMeter)te_r).GetSyntaxe(), m_x*20 + gui_terminal.tertminal_te.guiinfo.map_pos.x+ 20, m_y*20 + gui_terminal.tertminal_te.guiinfo.map_pos.y + 20, fluxmC);
									}
								}
							}
						}
					...
					else if(map.map[m_x][m_y] == 19)// flux name
					{
						if(!fnameAV)
						{
							if(m_x == mouse_mappos.x && m_y == mouse_mappos.y)//select item
							{
								if(map.maplink[m_x][m_y] != null)
								{
									...
									if(te_r != null && te_r instanceof Machine_FluidName)
									{
										gui_terminal.drawString(fontRendererObj, "FName: "+((Machine_FluidName)te_r).GetFluidName(), m_x*20 + gui_terminal.tertminal_te.guiinfo.map_pos.x+ 20, m_y*20 + gui_terminal.tertminal_te.guiinfo.map_pos.y + 20, fnameC);
									}
								}
							}
						}
						...
						{	
							if(m_x == mouse_mappos.x && m_y == mouse_mappos.y)//select item
							{
								if(map.maplink[m_x][m_y] != null)
								{
									...
									if(te_r != null && te_r instanceof Machine_FluidCounter)
									{
										gui_terminal.drawString(fontRendererObj, ((Machine_FluidCounter)te_r).GetSyntaxe(), m_x*20 + gui_terminal.tertminal_te.guiinfo.map_pos.x+ 20, m_y*20 + gui_terminal.tertminal_te.guiinfo.map_pos.y + 20, counterC);
									}
								}
							}
						}
						...
						{	
							if(m_x == mouse_mappos.x && m_y == mouse_mappos.y)//select item
							{
								if(map.maplink[m_x][m_y] != null)
								{
									...
									if(te_r != null && te_r instanceof Machine_FluidStorageInfo)
									{
										gui_terminal.drawString(fontRendererObj, ((Machine_FluidStorageInfo)te_r).GetSyntaxe(), m_x*20 + gui_terminal.tertminal_te.guiinfo.map_pos.x+ 20, m_y*20 + gui_terminal.tertminal_te.guiinfo.map_pos.y + 20, storageC);
									}
								}
							}
						}
		...
		if(gui_terminal.tertminal_te.guiinfo.editmode)
		{
			...
			//infogui.draw(classTag+"@item"+gui_terminal.tertminal_te.guiinfo.itemselected, gui_terminal.width - 48, 52, 0xffffff, this.gui_terminal, fontRendererObj);
			//System.out.println(classTag+":item"+gui_terminal.tertminal_te.guiinfo.itemselected);
		}
		...
		if(editlink != null)
		{
			...
			else if(tem instanceof Machine_FluidValve)
			{
				...
				gui_terminal.drawTexturedModalRect(x2, y2, 0, 0, 150, 40);
				gui_terminal.drawString(fontRendererObj, "Type: "+((Machine_FluidValve)tem).name(), x2+1, y2+1, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "CustomName: "+((Machine_FluidValve)tem).CustomName(), x2+1, y2+11, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "Pos: ["+tem.xCoord+","+tem.yCoord+","+tem.zCoord+"]", x2+1, y2+21, 0xffffff);
				gui_terminal.drawString(fontRendererObj, ((Machine_FluidValve)tem).GetSyntaxe(), x2+1, y2+31, 0xffffff);
			}
			else if(tem instanceof Machine_FluidCounter)
			{
				...
				gui_terminal.drawTexturedModalRect(x2, y2, 0, 0, 150, 40);
				gui_terminal.drawString(fontRendererObj, "Type: "+((Machine_FluidCounter)tem).name(), x2+1, y2+1, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "CustomName: "+((Machine_FluidCounter)tem).CustomName(), x2+1, y2+11, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "Pos: ["+tem.xCoord+","+tem.yCoord+","+tem.zCoord+"]", x2+1, y2+21, 0xffffff);
				gui_terminal.drawString(fontRendererObj, ((Machine_FluidCounter)tem).GetSyntaxe(), x2+1, y2+31, 0xffffff);
			}
			else if(tem instanceof Machine_FluidFlowMeter)
			{
				...
				gui_terminal.drawTexturedModalRect(x2, y2, 0, 0, 150, 40);
				gui_terminal.drawString(fontRendererObj, "Type: "+((Machine_FluidFlowMeter)tem).name(), x2+1, y2+1, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "CustomName: "+((Machine_FluidFlowMeter)tem).CustomName(), x2+1, y2+11, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "Pos: ["+tem.xCoord+","+tem.yCoord+","+tem.zCoord+"]", x2+1, y2+21, 0xffffff);
				gui_terminal.drawString(fontRendererObj, ((Machine_FluidFlowMeter)tem).GetSyntaxe(), x2+1, y2+31, 0xffffff);
			}
			else if(tem instanceof Machine_FluidStorageInfo)
			{
				...
				gui_terminal.drawTexturedModalRect(x2, y2, 0, 0, 150, 40);
				gui_terminal.drawString(fontRendererObj, "Type: "+((Machine_FluidStorageInfo)tem).name(), x2+1, y2+1, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "CustomName: "+((Machine_FluidStorageInfo)tem).CustomName(), x2+1, y2+11, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "Pos: ["+tem.xCoord+","+tem.yCoord+","+tem.zCoord+"]", x2+1, y2+21, 0xffffff);
				gui_terminal.drawString(fontRendererObj, ((Machine_FluidStorageInfo)tem).GetSyntaxe(), x2+1, y2+31, 0xffffff);
			}
			else if(tem instanceof Machine_FluidName)
			{
				gui_terminal.mc.renderEngine.bindTexture(ResourceLocationRegister.texture_pixel255);
				gui_terminal.setColorRGBA(0, 128, 128, 255);
				gui_terminal.drawTexturedModalRect(x2, y2, 0, 0, 150, 40);
				gui_terminal.drawString(fontRendererObj, "Type: "+((Machine_FluidName)tem).name(), x2+1, y2+1, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "CustomName: "+((Machine_FluidName)tem).CustomName(), x2+1, y2+11, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "Pos: ["+tem.xCoord+","+tem.yCoord+","+tem.zCoord+"]", x2+1, y2+21, 0xffffff);
				gui_terminal.drawString(fontRendererObj, "FName: "+((Machine_FluidName)tem).GetFluidName(), x2+1, y2+31, 0xffffff);
			}
		...
		if(!gui_terminal.tertminal_te.guiinfo.editmode)
		{
			if(click == 0 && mouse_mappos.x >= 0 && mouse_mappos.y >= 0 && mouse_mappos.x < map.map.length && mouse_mappos.y < map.map[0].length)
			{
				...
				if(vec != null && id >= 6 && id <= 11)
				{
					for(TileEntity te : machines)
					{
						if(vec != null && te instanceof Machine_FluidValve && te.xCoord == vec.x && te.yCoord == vec.y && te.zCoord == vec.z)
						{
							Machine_FluidValve sw = (Machine_FluidValve) te;
							sw.ValveOpenClose(!sw.ValveGetPosition());
						}
					}
				}
				else if(vec != null && id == 4)
				{
					for(TileEntity te : machines)
					{
						if(vec != null && te instanceof Machine_FluidCounter && te.xCoord == vec.x && te.yCoord == vec.y && te.zCoord == vec.z)
						{
							Machine_FluidCounter cnt = (Machine_FluidCounter) te;
							cnt.Reset();
						}
					}
				}
		...
		if(editlink != null)
		{
			if(cid == 200)//up
			{
				if(editlink.type == Type.Switch)
				{
					...
					for(TileEntity te : machines)
					{
						if(te instanceof Machine_FluidValve)machines_sp.add(te);
					}
				...
				else if(editlink.type == Type.FluxMetter)//TODO
				{
					List<TileEntity> machines_sp = new ArrayList<TileEntity>();
					for(TileEntity te : machines)
					{
						if(te instanceof Machine_FluidFlowMeter)machines_sp.add(te);
					}
				...
				else if(editlink.type == Type.Counter)
				{
					...
					for(TileEntity te : machines)
					{
						if(te instanceof Machine_FluidCounter)machines_sp.add(te);
					}
				...
				else if(editlink.type == Type.StorageInfo)
				{
					...
					for(TileEntity te : machines)
					{
						if(te instanceof Machine_FluidStorageInfo)machines_sp.add(te);
					}
			...
			else if(cid == 208)//down
			{
				...
				else if(editlink.type == Type.FluidName)
				{
					List<TileEntity> machines_sp = new ArrayList<TileEntity>();
					for(TileEntity te : machines)
					{
						if(te instanceof Machine_FluidName)machines_sp.add(te);
					}
	...
	short[] truelist = {0,1,2,3,4,5,8,9,12,13,14,15,16,17,18,19};
	int idselected = 0;
		...
		if(gui_terminal.tertminal_te.guiinfo.editmode)
		{
			if(Mouse.isButtonDown(2))//select ent (edit ent)
			{
				if(mouse_mappos.x >= 0 && mouse_mappos.x < map.map.length && mouse_mappos.y >= 0 && mouse_mappos.y < map.map[0].length)
				{
					...
					{
						editlink = new Tag(new Vector2(mouse_mappos.x, mouse_mappos.y), Tag.Type.FluidName);
		...
		for(int m_x = 0; m_x < map.map.length; ++m_x)
		{
			for(int m_y = 0; m_y < map.map[0].length; ++m_y)
			{
				...
				if(id >= 6 && id <= 11)//switch
				{
					...
					for(TileEntity te : machines)
					{
						if(link_ent != null && te instanceof Machine_FluidValve && te.xCoord == link_ent.x && te.yCoord == link_ent.y && te.zCoord == link_ent.z)
						{
							noexist = false;
							if(id == 6 || id == 8 || id == 10)
							{
								map.map[m_x][m_y] = (short) (((Machine_FluidValve)te).ValveGetPosition()? 6 : 8);
							}
							else
							{
								map.map[m_x][m_y] = (short) (((Machine_FluidValve)te).ValveGetPosition()? 7 : 9);
							}
						}
					}
